...
void ofApp::update(){
    ...
    //replace least played sound with most played sound
    int threshold = 42;
    for(int i = 0; i < inputCounter.size(); i++){
        
        if(inputCounter[i] >= threshold){
            int minBut = inputCounter[i];
            int worstButton = 0;
            for(int j = 0; j < inputCounter.size(); j++){
                if(inputCounter[j] < minBut){
                    minBut = inputCounter[j];
                    worstButton = j;
                }
                inputCounter[j] = 0;
            }
            soundPlayers[worstButton].unload();
            soundPlayers[worstButton].load(ofToString(i)+".wav");
        }
